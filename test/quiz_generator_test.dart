...
void main() {
  group('QuizGenerator', () {
    test('空題庫丟 StateError', () {
      expect(() => QuizGenerator().next([], {}), throwsStateError);
    });

    test('單一題庫可重複出同題', () {
      final pool = [findKana('か')!];
      final gen = QuizGenerator(Random(1));
      expect(gen.next(pool, {}, previous: pool.first).kana, 'か');
    });

    test('連續兩題不重複', () {
      final pool = PracticeMode.hiragana.buildPool(allKana);
      final gen = QuizGenerator(Random(42));
    ...
    test('熟練度超界 clamp 到 0..5', () {
      ...
      final gen = QuizGenerator(Random(9));
      // 不丟例外即可
